...
app = Flask(__name__)
serverip = 'https://makechaos.pythonanywhere.com'
...
headerTxt = '<div style="background-color:#b3d9ff;font-family:Arial;"><h1><span style="padding:10px;margin-left:auto;margin-right:auto;">A Project from Makechaos</span><h1></div>'
dateFormat = '%Y_%m_%d'
...
def addItems(txt):
    ...
    if len(ent)>0 and ent[-1]=='}':
      ...
      if name in items.keys():
        ...
        try:
            ...
            stats[name+'%s'%str(sys.exc_info())] = [[rs,-1], [amt, -1]]
  ...
  return 'Added %d items, total=%2.2f<br>%s'%(cnt,tot,ctime), getStatTable(stats)

def addHeader(htm):
     lbody = htm.find('<body')
     ebody = htm[lbody+1:].find('>')
     nins = lbody+ebody+2
     if htm[nins].find(headerTxt)<0:
        htm = htm[:nins] + headerTxt + htm[nins:]

     if htm.find('<title>')<0:
         htm = htm.replace('<head>','<head><title>A makechaos project</title>')
     return htm
...
def shoAllData(uid):
  ...
  udates = list(map(lambda x: datetime.datetime.strptime(x, dateFormat), udates))
...
@app.route('/multiply')
def multiplyTry():
  return render_html('multiplyTimer.html', {})
...
@app.route('/sudoku/update/<inp>', methods = ['GET'])
def sudokuUpdate(inp):
  return sudokuServer.update(inp)

@app.route('/sudoku/clear/<inp>', methods = ['GET'])
def sudokuClear(inp):
  return sudokuServer.clear(inp)

@app.route('/sudoku/poll/<dt>', methods = ['GET'])
def sudokuPoll(dt):
  return sudokuServer.poll(dt)

#********************************************** #
# Pose Server app
import poseServer

@app.route('/poses/upload', methods = ['GET', 'POST'])
def poseUploader():
  if request.method == 'POST':
    f = request.files['file']
    fname = 'tmp.csv'
    tempFile = os.path.join(ddir,'data',fname)
    f.save(tempFile)
  return poseServer.upload(tempFile)

@app.route('/poses')
def poseviewList():
  return poseServer.showAllEntries()

@app.route('/poses/view/<fname>')
def poseView(fname):
  return poseServer.showPose(fname)

@app.route('/poses/addtag/<fname>/<tags>')
def poseAddTag(fname, tags):
  return poseServer.saveTags(fname, json.loads(tags))
...
@app.route('/')
def masterPage():
    txt = '<html><head><meta name="viewport" content="width=device-width, user-scalable=no"><style>.h2{padding:10px;}</style></head><body>'
    txt+= '<h2><a href="http://makechaos.pythonanywhere.com/groc">Grocery App</a></h2>'
    txt+= '<h2><a href="http://makechaos.pythonanywhere.com/multiply">Multiplication Practice</a></h2>'
    txt+= '<h2><a href="http://makechaos.pythonanywhere.com/poses">Pose Viewer</a></h2>'
    txt+= '<h2><a href="http://makechaos.pythonanywhere.com/sudoku">Lets Sudoku</a></h2>'
    txt+= '</body></html>'
    return addHeader(txt)
